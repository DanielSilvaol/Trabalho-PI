...
public class ChatBDao {
	...
	public int cadastroAtendente(Atendente at) {
		String sqlInsert = "INSERT INTO atendente(Nome_Atendente, Email_Atendente, CPF_Atendente,"
		...
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			try (ResultSet rs = stm.executeQuery();) {
				if (rs.next()) {
					...
					pl.setPalavraChave(rs.getString("PalavraChave"));
				} else {
					pl.setId(-1);
					pl.setPalavraChave(null);
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
